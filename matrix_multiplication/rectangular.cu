...
__device__ Matrix GetSubMatrix(Matrix A, int row, int col, int width) {
	Matrix Asub;
	Asub.width = width;//BLOCK_SIZE;
	Asub.height = BLOCK_SIZE;
	Asub.stride = A.stride;
	Asub.elements = &A.elements[A.stride * BLOCK_SIZE * row + width * col];//BLOCK_SIZE * col];
	return Asub;
}
...
// Each thread block computes one sub-matrix Csub of C
	Matrix Csub = GetSubMatrix(C, blockRow, blockCol, 2*BLOCK_SIZE);
...
// Thread row and column within Csub
	...
	//if ((blockRow * blockDim.y + row < C.height) && (blockCol * blockDim.x + col < C.width)) {
		...
		for (int m = 0; m < (A.width / BLOCK_SIZE); ++m) {
			...
			Matrix Bsub = GetSubMatrix(B, m, blockCol, 2*BLOCK_SIZE);
			...
			Bs[row][col+BLOCK_SIZE] = GetElement(Bsub, row, col+BLOCK_SIZE);
			...
			for (int e = 0; e < BLOCK_SIZE; ++e) {
				...
				Dvalue += As[row][e] * Bs[e][col+BLOCK_SIZE];
			}
		...
		SetElement(Csub, row, col+BLOCK_SIZE, Dvalue);
	//}
}
